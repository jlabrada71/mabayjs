...
module.exports = class AppGenerator {
  ...
  async compilePrimitiveTemplates( primitives, templatesPath ) {
    ...
    for( let primitive of primitives ) {
      ...
      const template = await this.templateReader.fetch( primitive.key, primitive.type, templatesPath, primitiveFile );
  ...
  async compileCompositeTemplate( templatesPath , templateFile ) {
    ...
    const template = await this.templateReader.fetch( 'aggregate', 'composite', templatesPath, templateFile );
    templateCompiler.compile( template );
  ...
  async createCodeGenerator( model, manifesto, technologyPath, useCaseName ) {
    let compositeTemplate = await this.compileCompositeTemplate( technologyPath, manifesto.contextComposite );
    ...
    for( let context of manifesto.contexts ) {
      ...
      let aggregateTemplate = await this.compilePrimitiveTemplates( manifesto.primitives, contextPath);
