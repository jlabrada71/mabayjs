...
module.exports = class AppGenerator {
  ...
  async createCodeGenerator( model, manifesto, technologyPath, useCase ) {
    ...
    for( let context of manifesto.contexts ) {
      let contextPath = `${technologyPath}/${useCase}/${context}`;
  ...
  async generate( file, templatesPath, manifestoFile, outputPath) {
    ...
    for( let useCase of manifesto.useCases ) {
      ...
      let compositeCodeGenerator = await this.createCodeGenerator( model, manifesto, technologyPath, useCase );
      ...
      for( let aggregate in model.definitions.aggregates ) {
        ...
        const fileName = fileNameTemplate({ entityName: aggregate, usecase: useCase });

        await codeWriter.write( fileName, aggregateCode, outputPath, manifesto.folder, manifesto.ext );
