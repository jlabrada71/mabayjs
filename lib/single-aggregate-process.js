...
module.exports = class SingleAggregateProcess {
  ...
  async generateCode() {
    ...
    for( let aggregate in this.model.definitions.aggregates ) {
      console.log( "****** aggregate to generate:" + JSON.stringify(aggregate) +":" + JSON.stringify(this.model.definitions.aggregates[aggregate]) );
      ...
      const fileName = fileNameTemplate({ entityName: aggregate, usecase: this.useCase.name });

      await this.codeWriter.write( fileName, aggregateCode, this.outputPath, this.useCase.folder, this.manifesto.ext );
